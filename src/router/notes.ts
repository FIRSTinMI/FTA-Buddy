...
export const notesRouter = router({
	...
	getAllByAuthor: eventProcedure
		...
		.query(async ({ ctx, input }) => {
			const notesByAuthor = await db.query.notes.findMany({
				where: and(eq(notes.author_id, input.author_id)),
				orderBy: [asc(notes.created_at)],
			});
	...
	getAllByTeam: eventProcedure
		...
		.query(async ({ ctx, input }) => {
			const notesByTeam = await db.query.notes.findMany({
				where: and(eq(notes.team, input.team_number)),
				orderBy: [asc(notes.created_at)],
			});
	...
	generateNotesReport: eventProcedure.query(async ({ ctx }) => {
		...
		const path = await generateReport(
			...
			allNotes.map((note) => [note.team, note.text, note.author?.username ?? "Unknown"]),
			event.code,
		);
