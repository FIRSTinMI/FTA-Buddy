...
db.configure(config_1.DATABASE_CONFIG);
...
function updateTeamsListFromMonitor(event, monitor) {
    ...
    if (newTeamsAdded)
        db.query('UPDATE events SET teams = ? WHERE code = ?;', [JSON.stringify(events[event].teams), event]);
}
app.use((0, body_parser_1.json)());
app.use((0, cors_1.default)());
app.use(express_1.default.static('./cloud/public'));
app.get('/', (req, res) => {
    res.redirect('/app/');
});
...
app.post('/register/:event', (req, res) => {
    if (events[req.params.event]) {
        ...
        events[req.params.event] = {
            ip: req.body.ip,
            socketClients: [],
            monitor: constants_1.DEFAULT_MONITOR,
            teams: []
        };
    }
...
app.get('/monitor/:event', (req, res) => {
    if (!events.hasOwnProperty(req.params.event))
        return res.status(404).send();
    ...
    db.query('SELECT * FROM messages WHERE team = ?;', [req.params.team]).spread((messages) => {
        db.query(`SELECT * FROM profiles WHERE id IN (SELECT profile FROM messages WHERE team = ?);`, [req.params.team]).spread((profiles_raw) => {
            ...
            for (let m of messages) {
                messagesWithUsernames.push(Object.assign(Object.assign({}, m), { username: profiles[m.profile].username }));
            }
            res.send(messages);
        });
    });
