<script lang="ts">
	...
	function buildOption(): ECOption {
		...
		const yAxes = yAxisDefaults.map((def, i) => {
			...
			const axisVisible =
				i === 5 ? false : i < metricSeries.length ? visibleSeries[metricSeries[i].name] && def.show : false;
			return {
				type: "value" as const,
			...
			tooltip: {
				...
				formatter: (params: any) => {
					...
					if (frame.eStopPressed) {
						...
						status =
							'<span style="color:#22c55e">Enabled' + (frame.auto ? " (Auto)" : " (Teleop)") + "</span>";
					}
