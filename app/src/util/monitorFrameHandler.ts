...
export class MonitorFrameHandler extends EventTarget {
	...
	public fieldStatusChange(field: FieldState) {
		if (field === FieldState.PRESTART_COMPLETED) {
			this.dispatchEvent(
				new CustomEvent("prestart", { detail: { frame: this.getFrame(), match: MatchState.PRESTART } }),
			);
		} else if (field === FieldState.MATCH_OVER) {
			this.dispatchEvent(
				new CustomEvent("match-over", { detail: { frame: this.getFrame(), match: MatchState.OVER } }),
			);
		} else if (field === FieldState.MATCH_RUNNING_AUTO) {
			this.dispatchEvent(
				new CustomEvent("match-start", { detail: { frame: this.getFrame(), match: MatchState.RUNNING } }),
			);
		} else if (field === FieldState.MATCH_READY) {
			this.dispatchEvent(
				new CustomEvent("match-ready", { detail: { frame: this.getFrame(), match: MatchState.RUNNING } }),
			);
		}
	}
	...
	public robotStatusChange(change: StateChange) {
		...
		if (change.key === "ds") {
			this.dispatchEvent(
				new CustomEvent(change.type === StateChangeType.RisingEdge ? "ds-connect" : "ds-drop", {
					detail: { robot, frame, match },
				}),
			);
		} else if (change.key === "radio") {
			this.dispatchEvent(
				new CustomEvent(change.type === StateChangeType.RisingEdge ? "radio-connect" : "radio-drop", {
					detail: { robot, frame, match },
				}),
			);
		} else if (change.key === "rio") {
			this.dispatchEvent(
				new CustomEvent(change.type === StateChangeType.RisingEdge ? "rio-connect" : "rio-drop", {
					detail: { robot, frame, match },
				}),
			);
		} else if (change.key === "code") {
			this.dispatchEvent(
				new CustomEvent(change.type === StateChangeType.RisingEdge ? "code-connect" : "code-drop", {
					detail: { robot, frame, match },
				}),
			);
		}
	...
	public process() {
		...
		const currentMatchState = [
			FieldState.MATCH_RUNNING_TELEOP,
			FieldState.MATCH_RUNNING_AUTO,
			FieldState.MATCH_TRANSITIONING,
		].includes(currentFrame.field)
			? MatchState.RUNNING
			: [
						FieldState.MATCH_OVER,
						FieldState.MATCH_ABORTED,
						FieldState.READY_FOR_POST_RESULT,
						FieldState.READY_TO_PRESTART,
				  ].includes(currentFrame.field)
				? MatchState.OVER
				: MatchState.PRESTART;
		...
		for (let _robot in ROBOT) {
			...
			if (currentRobot.lastChange) {
				if (currentRobot.ds === DSState.RED) {
					if (this.alertsPlayed[robot].ds || currentRobot.lastChange.getTime() + 45e3 > new Date().getTime())
						continue;
					this.alertsPlayed[robot].ds = true;
					this.dispatchEvent(
						new CustomEvent("ds-alert", {
							detail: { robot, frame: currentFrame, match: currentMatchState },
						}),
					);
				} else if (currentRobot.ds === DSState.GREEN_X) {
					if (this.alertsPlayed[robot].ds || currentRobot.lastChange.getTime() + 30e3 > new Date().getTime())
						continue;
					this.alertsPlayed[robot].ds = true;
					this.dispatchEvent(
						new CustomEvent("ds-alert", {
					...
					if (
						this.alertsPlayed[robot].radio ||
						currentRobot.lastChange.getTime() + 240e3 > new Date().getTime()
					)
						continue;
					this.alertsPlayed[robot].radio = true;
					this.dispatchEvent(
						new CustomEvent("radio-alert", {
							detail: { robot, frame: currentFrame, match: currentMatchState },
						}),
					);
				} else if (!currentRobot.rio) {
					if (this.alertsPlayed[robot].rio || currentRobot.lastChange.getTime() + 45e3 > new Date().getTime())
						continue;
					this.alertsPlayed[robot].rio = true;
					this.dispatchEvent(
						new CustomEvent("rio-alert", {
							detail: { robot, frame: currentFrame, match: currentMatchState },
						}),
					);
				} else if (!currentRobot.code) {
					if (
						this.alertsPlayed[robot].code ||
						currentRobot.lastChange.getTime() + 30e3 > new Date().getTime()
					)
						continue;
					this.alertsPlayed[robot].code = true;
					this.dispatchEvent(
						new CustomEvent("code-alert", {
							detail: { robot, frame: currentFrame, match: currentMatchState },
						}),
					);
				}
