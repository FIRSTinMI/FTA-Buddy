...
export const cycleRouter = router({
	postCycleTime: publicProcedure
		...
		.mutation(async ({ input }) => {
			...
			let cycle: any = await db.query.cycleLogs.findFirst({
				where: and(
					eq(cycleLogs.event, event.code),
					eq(cycleLogs.match_number, input.matchNumber),
					eq(cycleLogs.play_number, input.playNumber),
					eq(cycleLogs.level, input.level),
				),
			});
			...
			if (input.type === "lastCycleTime" && input.lastCycleTime) {
				...
				redis.set(`ftabuddy:event:${event.code}:monitor_frame`, SuperJSON.stringify(frame)).catch(() => {});
				await db
					.update(cycleLogs)
	...
	getCycle: publicProcedure
		...
		.query(async ({ input }) => {
			const cycle = await db.query.cycleLogs.findFirst({
				where: and(
					eq(cycleLogs.event, input.eventCode),
					eq(cycleLogs.match_number, input.matchNumber),
					eq(cycleLogs.play_number, input.playNumber),
					eq(cycleLogs.level, input.level),
				),
	...
	postScheduleDetails: eventProcedure
		.input(
			z.object({
				eventToken: z.string(),
				days: z.array(
		...
		.mutation(async ({ input }) => {
			...
			if (input.matches) {
				...
				if (!frame.exactAheadBehind && timing.lastMatchStart) {
					const scheduled = input.matches.find((m) => m.match === frame.match && m.level === frame.level);
					if (scheduled) {
	...
	generateCycleTimeReport: eventProcedure.query(async ({ ctx }) => {
		const cycles = await db
			.select()
			.from(cycleLogs)
			.where(and(eq(cycleLogs.event, ctx.event.code), isNotNull(cycleLogs.start_time)))
...
async function getAverageCycleTime(eventCode: string, rollingAverage: number = 10) {
	const cycles = await db.query.cycleLogs.findMany({
		where: eq(cycleLogs.event, eventCode),
		limit: rollingAverage < 0 ? 10000 : rollingAverage * 3,
		orderBy: desc(cycleLogs.start_time),
	});
