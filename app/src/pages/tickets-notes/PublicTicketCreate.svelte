...
<Modal bind:open={termsPopupOpen} size="sm" dialogClass="fixed top-0 start-0 end-0 h-modal md:inset-0 md:h-full z-40 w-full p-4 flex">
	<h1 class="font-bold text-2xl">Terms and Conditions</h1>
    <p class="text-center">
        You must read and agree to these terms before you may submit a ticket.
    </p>
    <p class="text-center">
        All Tickets are saved after the end of the event and can be viewed by volunteers at other events the team attends in the future.
    </p>
    <p class="text-center">
    ...
    <p class="text-center">
        If Ticket creation on this public site is abused in any way, the volunteers will disable it. Be respectful; please do not spam.
    </p>
    <Button on:click={() => {
        agreeTerms = true;
    ...
    <Button on:click={() => {
        termsPopupOpen = false;
        toast("You must agree to the terms to create a Ticket", "failure", "red-500")
    }}>Do Not Agree</Button>
</Modal>
