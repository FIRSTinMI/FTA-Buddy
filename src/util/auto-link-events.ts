...
import type { Message, Note, Profile, ServerEvent } from "../../shared/types";
 ...
 */
function formatIssue(d: { issue: string; start_time: number | null; end_time: number | null; duration: number | null }): string {
    const dur = Math.abs(d.duration ?? 0);
    const durLabel = dur >= 60 ? `${(dur / 60).toFixed(1)}m` : `${dur.toFixed(0)}s`;
    const timeRange =
        d.start_time != null && d.end_time != null
            ? ` (${d.start_time}s \u2192 ${d.end_time}s match time)`
            : "";
...
export async function autoLinkEventsToNote(
    noteId: string,
    eventRows: { id: string; issue: string; issues: any; start_time: number | null; end_time: number | null; duration: number | null }[],
    event: ServerEvent,
) {
    ...
    // Build summary of auto-linked issues
    const issueDescriptions = eventRows.flatMap((e) => {
        const issueList =
            (e.issues as { issue: string; start_time: number | null; end_time: number | null; duration: number | null }[] | null) ??
    ...
    // Add a system message to the note
    ...
    await db.update(notes).set({ updated_at: new Date() }).where(eq(notes.id, noteId));
...
export async function tryAutoLinkNewMatchEvent(
    inserted: { id: string; issue: string; issues: any; start_time: number | null; end_time: number | null; duration: number | null; team: number; match_number: number },
    eventCode: string,
    serverEvent: ServerEvent,
): Promise<boolean> {
    // Find the most recently updated open note for this team + match
    const openNote = await db.query.notes.findFirst({
        where: and(
            eq(notes.event_code, eventCode),
            eq(notes.team, inserted.team),
            eq(notes.match_number, inserted.match_number),
