...
export async function generateReport(format: ReportFormat, data: (string | number)[][], eventCode: string) {
	...
	for (let i = 1; i <= pageCount; i++) {
		doc.setPage(i);
		doc.text(`${eventCode}`, doc.internal.pageSize.width / 2, doc.internal.pageSize.height - 12, {
			align: "center",
		});
		doc.text(`Generated by FTA Buddy`, doc.internal.pageSize.width - 10, doc.internal.pageSize.height - 15, {
			align: "right",
		});
		doc.text(formattedDate, doc.internal.pageSize.width - 10, doc.internal.pageSize.height - 10, {
			align: "right",
		});
	}
