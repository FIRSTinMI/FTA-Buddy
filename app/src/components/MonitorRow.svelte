...
{#key robot}
<button
    class="w-full fieldmonitor-square-height md:aspect-square flex flex-col px-1 items-center justify-center text-lg sm:text-2xl lg:text-4xl {fullscreen && "lg:text-6xl"} font-mono tabular-nums {getKey(robot)?.startsWith('blue') ? 'bg-blue-600' : 'bg-red-600'}"
    on:click={() => navigate("/app/messages#notes-" + robot.number)}
>
    <p>{robot.number}</p>
...
<button
    class="{Status_Colors[robot.radio ? 1 : 0]} fieldmonitor-square-height md:aspect-square flex"
    on:click={detailView}
    id="{getKey(robot)}-radio"
></button>
<button
    class="{Status_Colors[(robot.rio ? 1 : 0)]} fieldmonitor-square-height md:aspect-square flex items-center justify-center font-mono text-4xl lg:text-8xl text-black"
    on:click={detailView}
    id="{getKey(robot)}-rio"
>
...
<button
    ...
    on:click={detailView}
    style="background-color: rgba(255,0,0,{robot.battery < 11 &&
    robot.battery > 0
...
<button
    ...
    on:click={detailView}
    style="background-color: rgba(255,0,0,{robot.ping >= 20 &&
    robot.ping < 100
        ? Math.log10(robot.ping/25)
        : (robot.ping > 100 ? 0.5 : 0)})"
...
<button on:click={() => detailView} class="fieldmonitor-square-height hidden lg:flex items-end pb-2 justify-center text-md sm:text-xl lg:text-4xl {fullscreen && "lg:text-5xl"} tabular-nums" id="{getKey(robot)}-bwu">
    {robot.bwu.toFixed(2)}
</button>
<button class="fieldmonitor-square-height hidden lg:flex md:aspect-square flex-col items-center justify-end" on:click={() => detailView} id="{getKey(robot)}-signal">
    {robot.signal ?? ""}
...
<button on:click={() => detailView} class="fieldmonitor-square-height hidden lg:flex items-end pb-2 justify-center text-md sm:text-xl lg:text-4xl {fullscreen && "lg:text-5xl"} tabular-nums" id="{getKey(robot)}-time">
    {(!robot.code && robot.lastChange) ? formatTimeShortNoAgoSecondsOnly(robot.lastChange) : ""}
</button>
<button on:click={() => detailView} class="fieldmonitor-square-height lg:hidden flex flex-col items-end justify-center tabular-nums" id="{getKey(robot)}-net">
