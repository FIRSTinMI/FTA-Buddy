...
export const ticketsRouter = router({
	...
	getAllByWithMessages: eventProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			if (input.event_code) {
				...
				if (event.meshedEvent && event.subEvents) {
					query.push(
						inArray(tickets.event_code, [event.code, ...event.subEvents.map((subEvent) => subEvent.code)]),
					);
				} else {
	...
	getByIdWithMessages: eventProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const ticket = await db.query.tickets.findFirst({
				where: and(eq(tickets.id, input.id), inArray(tickets.event_code, eventCodes)),
				with: {
					messages: {
						orderBy: [asc(messages.id)],
					},
	...
	publicCreate: publicProcedure
		...
		.mutation(async ({ input }) => {
			...
			if (!event.publicTicketSubmit) {
				throw new TRPCError({
					code: "BAD_REQUEST",
					message: "Public Ticket submission is disabled for this Event",
				});
			}
			...
			if (event.slackChannel && event.slackTeam) {
				...
				buttons.push({
					...
					url: `https://ftabuddy.com/tickets/view/${insert[0].id}`,
				});
				if (insert[0].match_id) {
					buttons.push({
						...
						url: `https://ftabuddy.com/logs/${insert[0].match_id}`,
					});
				}

				let messageTS = await sendSlackMessage(
					event.slackChannel,
					event.slackTeam,
					createSlackTicketMessage(
						insert[0].id,
						insert[0].team,
						"Team Submission",
						insert[0].subject,
				...
				await db
					...
					.where(eq(tickets.id, insert[0].id))
					.execute();
			}
			...
			if (event.slackChannel && event.slackTeam) {
				let messageTS = await sendSlackMessage(
					...
					createSlackTicketMessage(
						insert[0].id,
						insert[0].team,
						insert[0].author?.username ?? "Unknown",
						insert[0].subject,
						insert[0].text,
						insert[0].match_id ?? undefined,
					),
	...
	updateStatus: eventProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const ticket = await db.query.tickets.findFirst({
				where: and(eq(tickets.event_code, input.event_code), eq(tickets.id, input.id)),
			});
	...
	assign: eventProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const ticket = await db.query.tickets.findFirst({
				where: and(eq(tickets.id, input.id), eq(tickets.event_code, input.event_code)),
			});
			...
			createNotification(
				ticket.followers.filter((id) => id !== profile[0].id),
				{
					id: randomUUID(),
					timestamp: new Date(),
	...
	unAssign: eventProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const ticket = await db.query.tickets.findFirst({
				where: and(eq(tickets.id, input.ticket_id), eq(tickets.event_code, input.event_code)),
			});
			...
			if (ticket.assigned_to === null && ticket.assigned_to_id) {
				profile = await db
					...
					.from(users)
					.where(eq(users.id, ticket.assigned_to_id));
	...
	follow: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (!followers.includes(ctx.user.id) && input.follow === true) {
				...
				throw new TRPCError({
					code: "BAD_REQUEST",
					message: "Current User is already following provided Ticket",
				});
			} else if (followers.includes(ctx.user.id) && input.follow === false) {
	...
	generateTicketReport: eventProcedure.query(async ({ ctx }) => {
		...
		const path = await generateReport(
			...
			allTeamsTicketInfo.map((teamInfo) => [
				...
				teamInfo.longestTicket !== null
					? `#${teamInfo.longestTicket.id} - ${teamInfo.longestTicket.subject}`
					: "",
			...
			if (event.slackTeam && ticket.slack_ts && ticket.slack_channel) {
				await updateSlackMessage(
					...
					createSlackTicketMessage(
						ticket.id,
						ticket.team,
						ticket.author?.username ?? "Unknown",
						ticket.subject,
						ticket.text,
...
export async function getTeamTicketsInfo(event_code: string, team: number) {
	let teamTicketArray = (await db.query.tickets.findMany({
		where: and(eq(tickets.event_code, event_code), eq(tickets.team, team)),
		orderBy: [desc(tickets.created_at)],
	})) as Ticket[];
	...
	const ticketLinks: string[] = teamTicketArray.map((ticket) => `https://www.ftabuddy.com/tickets/view/${ticket.id}`);
...
export async function getAllTeamTicketsInfo(event_code: string, teamNumbers: number[]) {
	...
	const filteredAllTeamsTicketInfo = sortedAllTeamsTicketInfo.filter(
		(teamTicketInfo) => teamTicketInfo.tickets.length !== 0,
	);
...
export function getLongestTicketOpenTime(tickets: Ticket[]) {
	...
	if (closedTickets.length <= 1) {
		...
		const openTime =
			longestTicket.closed_at && longestTicket.created_at
				? longestTicket.closed_at.getTime() - longestTicket.created_at.getTime()
				: 0;
...
export async function updateTicketStatusFromSlack(message_ts: string, status: boolean) {
	const ticket = await db.query.tickets.findFirst({
		where: and(eq(tickets.slack_ts, message_ts)),
	});
...
export function createSlackTicketMessage(
	ticket_id: number,
	team_number: number,
	author: string,
	subject: string,
	...
	buttons.push({
		...
		url: `https://ftabuddy.com/tickets/view/${ticket_id}`,
	});
	if (match_id) {
		buttons.push({
			type: "button",
