...
const audioClips: AudioClips = {
	...
	green: ["audio-green1.ogg", "audio-green2.ogg", "audio-green3.ogg"],
	other: {
...
export class AudioQueuer {
	private queue: {
		...
		clip: "ds" | "radio" | "rio" | "code" | "astop" | "estop";
	}[] = [];
	...
	public addRobotClip(robot: ROBOT, clip: "ds" | "radio" | "rio" | "code" | "astop" | "estop") {
		...
		for (const existingClip of existingClipsForRobot) {
			...
			if (clip === "code") {
				if (existingClip.clip !== "ds" && existingClip.clip !== "radio" && existingClip.clip !== "rio")
					remove = true;
				else return;
			}
	...
	public addGreenClip() {
		...
		this.queue.push({
			audio: this.getClip(audioClips.green[Math.floor(Math.random() * audioClips.green.length)]),
			robot: undefined,
			clip: "ds",
		});
		if (!this.playing) this.playNext();
