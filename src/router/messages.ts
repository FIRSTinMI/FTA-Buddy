...
import { adminProcedure, eventProcedure, router } from "../trpc";
...
export const messagesRouter = router({
	loadAllForEvent: adminProcedure
		.input(
			z.object({
				event_code: z.string(),
			}),
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (!insert[0]) {
				throw new TRPCError({
					...
					message: "Unable to create Message on this Ticket",
				});
			}
	...
	edit: eventProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const ticket = await db.query.tickets.findFirst({
				where: and(eq(tickets.id, input.ticket_id), eq(tickets.event_code, event.code)),
			});
...
export async function addTicketMessageFromSlack(
	channel_id: string,
	message_ts: string,
	thread_ts: string,
	text: string,
