...
export function updateScheduleText(
	...
	if (lunchMatch && currentMatch <= lunchMatch) {
		...
		let end = formatTimeShortNoAgoMinutesOnly(
			endTime,
			new Date(scheduleDetails.days[currentScheduleDay].endTime ?? fallbackEndOfDay),
		);
		...
		let aheadBehind =
			endTime.getTime() <=
			new Date(scheduleDetails.days[currentScheduleDay].endTime ?? fallbackEndOfDay).getTime();
