...
{#key team}
    <TableBodyRow class="h-20 border-y border-gray-800 cursor-pointer" id="{station}-row">
        <TableBodyCell class="{getKey(team)?.startsWith('blue') ? 'bg-blue-600' : 'bg-red-600'} font-mono" on:click={navigate("/app/notes/" + team.number)}>
            {team.number}
        </TableBodyCell>
        ...
        <TableBodyCell class="{RIO_Colors[team.rio + team.code]} border-x border-gray-800" on:click={detailView}></TableBodyCell>
