...
export const cycleRouter = router({
    ...
    generateCycleTimeReport: eventProcedure.query(async ({ ctx }) => {
        ...
        doc.setFontSize(20);
        doc.text(`Cycle Time Report for ${ctx.event.code}`, 15, 20);
        doc.setFontSize(12);
        doc.text(`Average Cycle Time: ${formatTimeShortNoAgoSeconds(averageCycleTime)}`, 15, 30);

        autoTable(doc, {
            head: [['Match Number', 'Play Number', 'Level', 'Start Time', 'Cycle Time']],
            body: cycles.map(cycle => [
                cycle.match_number,
                cycle.play_number,
                cycle.level,
        ...
        for (let i = 1; i <= pageCount; i++) {
            doc.setPage(i);
            doc.text(`${ctx.event.code}`, doc.internal.pageSize.width / 2, doc.internal.pageSize.height - 12, { align: 'center' });
        ...
        await doc.save(`reports/CycleTimeReport-${ctx.event.code}.pdf`);

        return { path: `/report/CycleTimeReport-${ctx.event.code}.pdf` };
    }),
});
