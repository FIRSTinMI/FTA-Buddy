	...
	<div class="relative overflow-x-auto rounded-lg bg-neutral-50 dark:bg-neutral-700">
		<table class="w-full text-center text-sm">
			<thead class="bg-neutral-100 dark:bg-neutral-800">
				<tr>
					<th class="px-4 py-2 hidden md:table-cell sticky left-0 z-10 bg-neutral-100 dark:bg-neutral-800">Match</th>
					<th class="px-4 py-2 hidden md:table-cell sticky left-0 z-10 bg-neutral-100 dark:bg-neutral-800">Play</th>
				...
				{#each filteredMatches as match}
					<tr class="cursor-pointer hover:bg-neutral-100 dark:hover:bg-neutral-600">
						...
						<td
							...
							onclick={() => navigate("/logs/:matchid/:station", { params: { matchid: match.id, station: "blue1" } })}
							>{match.blue1 ?? "None"}{#if match.blue1_has_event}<span class="ml-1">⚙️</span>{/if}{#if match.blue1_bypassed}<span class="ml-1">🚫</span>{/if}</td
						>
						<td
							class="px-1 py-2 bg-blue-400 dark:bg-blue-600 hover:bg-opacity-50 hover:underline"
							onclick={() => navigate("/logs/:matchid/:station", { params: { matchid: match.id, station: "blue2" } })}
							>{match.blue2 ?? "None"}{#if match.blue2_has_event}<span class="ml-1">⚙️</span>{/if}{#if match.blue2_bypassed}<span class="ml-1">🚫</span>{/if}</td
						>
						<td
							class="px-1 py-2 bg-blue-400 dark:bg-blue-600 hover:bg-opacity-50 hover:underline"
							onclick={() => navigate("/logs/:matchid/:station", { params: { matchid: match.id, station: "blue3" } })}
							>{match.blue3 ?? "None"}{#if match.blue3_has_event}<span class="ml-1">⚙️</span>{/if}{#if match.blue3_bypassed}<span class="ml-1">🚫</span>{/if}</td
						>
						<td
							class="px-1 py-2 bg-red-400 dark:bg-red-600 hover:bg-opacity-50 hover:underline"
							onclick={() => navigate("/logs/:matchid/:station", { params: { matchid: match.id, station: "red1" } })}
							>{match.red1 ?? "None"}{#if match.red1_has_event}<span class="ml-1">⚙️</span>{/if}{#if match.red1_bypassed}<span class="ml-1">🚫</span>{/if}</td
						>
						<td
							class="px-1 py-2 bg-red-400 dark:bg-red-600 hover:bg-opacity-50 hover:underline"
							onclick={() => navigate("/logs/:matchid/:station", { params: { matchid: match.id, station: "red2" } })}
							>{match.red2 ?? "None"}{#if match.red2_has_event}<span class="ml-1">⚙️</span>{/if}{#if match.red2_bypassed}<span class="ml-1">🚫</span>{/if}</td
						>
						<td
							class="px-1 py-2 bg-red-400 dark:bg-red-600 hover:bg-opacity-50 hover:underline"
							onclick={() => navigate("/logs/:matchid/:station", { params: { matchid: match.id, station: "red3" } })}
							>{match.red3 ?? "None"}{#if match.red3_has_event}<span class="ml-1">⚙️</span>{/if}{#if match.red3_bypassed}<span class="ml-1">🚫</span>{/if}</td
						>
						<td
